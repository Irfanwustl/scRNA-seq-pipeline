...
class BatchCorrectionStep:
    """
    Step wrapper for `apply_batch_correction`.

    Runs a batch correction method (or no-op) on an AnnData that already has
    a PCA embedding in `adata.obsm[rep_in]`.

    Records the representation name to use downstream in:
    `adata.uns["scrna_pipeline"]["batch_correction"]["rep_used"]`.
    """

    name: str = "batch_correction"

    method: str = "none"
    batch_key: str | None = "sample"
    rep_in: str = "X_pca"

    # Extra method-specific parameters (e.g. rep_out)
    params: Optional[Dict[str, Any]] = None

    def run(self, adata: AnnData, ctx) -> AnnData:
        kwargs = dict(self.params or {})

        rep_used = apply_batch_correction(
            adata,
            method=self.method,
            batch_key=self.batch_key,
            rep_in=self.rep_in,
            verbose=getattr(ctx, "verbose", True),
            **kwargs,
        )

        # provenance / contract
        adata.uns.setdefault("scrna_pipeline", {})
        adata.uns["scrna_pipeline"].setdefault("batch_correction", {})
        adata.uns["scrna_pipeline"]["batch_correction"].update({
            "method": self.method,
            "batch_key": self.batch_key,
            "rep_in": self.rep_in,
            "rep_used": rep_used,
    ...
    def outputs(self) -> Tuple[str, ...]:
        return ("uns[scrna_pipeline/batch_correction/rep_used]",)
