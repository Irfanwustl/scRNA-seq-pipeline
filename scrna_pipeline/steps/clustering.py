...
class ClusterAndEmbedStep:
    """
    Step wrapper for `cluster_and_embed`.

    Notes
    -----
    - This Step can be turned off by setting `clustering_method="none"` or by
      excluding it from the workflow step list.
    - If you want to avoid writing large graphs to disk, drop `adata.obsp`
      at SAVE time in the workflow (recommended).
    """
    name: str = "cluster_and_embed"
    ...
    def outputs(self) -> Tuple[str, ...]:
        outs = ("uns[neighbors]", "obsp[distances]", "obsp[connectivities]", "obsm[X_umap]")
        # cluster key depends on method; we can't know exact key_added if None, so keep generic
        return outs
