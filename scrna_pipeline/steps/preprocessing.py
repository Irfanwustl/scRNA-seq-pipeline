...
class PreprocessToPCAStep:
    """
    Step wrapper for `preprocess_to_pca`.

    Notes
    -----
    - This is intentionally a thin wrapper around the tested function.
    - Use `params` to pass overrides without growing the Step signature forever.
    """
    name: str = "preprocess_to_pca"

    # Common knobs you almost always set:
    batch_key: str | None = "sample"
    hvg_flavor: str = "seurat_v3"
    n_top_genes: int = 2000
    n_pcs: int = 50

    # Optional extra params forwarded into preprocess_to_pca
    params: Optional[Dict[str, Any]] = None

    def run(self, adata: AnnData, ctx) -> AnnData:
        kwargs = dict(self.params or {})
        return preprocess_to_pca(
            adata,
